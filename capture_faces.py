...
class FaceCollector:
    ...
    def collect(self, name):
        dirs = {'raw': f"dataset/{name}/raw", 'processed': f"dataset/{name}/processed"}
        ...
        paused = False
        print(f"📸 Collecting faces for {name} (Q-quit, SPACE-pause, R-reset)")
        ...
        while count < self.MAX_IMAGES:
            if not paused:
                ...
                if len(faces)==1:
                    ...
                    if in_circle:
                        if area < self.MIN_AREA: msg="📏 Move closer"
                        elif area > self.MAX_AREA: msg="📏 Move farther"
                        else:
            ...
            if key==ord('q'): break
            elif key==ord(' '): paused=not paused; print("⏸️ Paused" if paused else "▶️ Resumed")
            elif key==ord('r'): count=0; stats={'saved':0,'rejected':0}; print("🔄 Reset")
        
        cap.release()
        cv2.destroyAllWindows()
        
        metadata={'name':name,'date':datetime.now().isoformat(),'images':count,'stats':stats}
        with open(f"dataset/{name}/metadata.json",'w') as f: json.dump(metadata,f,indent=2)
        print(f"\n✅ Collected {count} images for {name} | Saved: {stats['saved']}, Rejected: {stats['rejected']}")
        
        # Auto-run encode_faces.py
        if count>0:
            print("📦 Running encode_faces.py ...")
            try:
                subprocess.run([os.sys.executable, "encode_faces.py"], check=True)
                print("✅ encode_faces.py finished successfully")
            except subprocess.CalledProcessError as e:
                print(f"❌ encode_faces.py failed: {e}")
